...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Pre-roll before each chime starts (ms): a fixed lead so the audio
		*  hardware/context is fully awake before the first tone — no attack edge
		*  is ever clipped, especially right after an idle auto-suspend. */
		var PRE_ROLL_MS = 500;
		...
		*  PRE_ROLL_MS lead keeps every start safely in the future so no attack
		*  edge is ever clipped by a live clock or a just-resumed context. */
		function startSequence(ctx, preset, vol, count, gap) {
			var base = ctx.currentTime + PRE_ROLL_MS / 1000;
		...
		*  silently miss a ring. */
		function playCustomAudio(url, volume, repeat, interval) {
			...
			// Same pre-roll as the synthetic path: every ring starts after
			// PRE_ROLL_MS so the first tone never gets clipped.
			pendingTimers.push(setTimeout(playOnce, PRE_ROLL_MS));
			for (var i = 1; i < count; i++) {
				pendingTimers.push(setTimeout(playOnce, PRE_ROLL_MS + i * gap));
			}
		}
