...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*  first interaction so later chimes are never blocked. */
		...
		/** One sine blip with a fast attack / exponential decay envelope. */
		function tone(ctx, freq, at, dur, vol) {
		...
		/** Pending repeat-sequence timers; a new sequence cancels the old one. */
		var pendingTimers = [];
		...
		/** One ring of a preset (called once per repeat). */
		function ringOnce(ctx, preset, vol) {
			if (preset === "falling") {
				// 880 -> 620
				tone(ctx, 880, 0, 0.16, vol);
				tone(ctx, 620, 0.16, 0.22, vol);
			} else if (preset === "dingdong") {
				// ding-dong: high short "ding", low longer "dong"
				tone(ctx, 880, 0, 0.16, vol);
				tone(ctx, 660, 0.18, 0.32, vol);
			} else if (preset === "beep") {
				// single short blip
				tone(ctx, 880, 0, 0.2, vol);
			} else {
				// rising: 620 -> 880 (default)
				tone(ctx, 620, 0, 0.18, vol);
				tone(ctx, 880, 0.16, 0.24, vol);
			}
		}
		...
		*  repeats of one sequence are never blocked by it. */
		function playPreset(preset, volume, repeat, interval) {
			...
			ringOnce(ctx, preset, vol);
			for (var i = 1; i < count; i++) {
				pendingTimers.push(setTimeout(function () {
					var c = ensureContext();
					if (c) ringOnce(c, preset, vol);
				}, i * gap));
			}
		}
		//#endregion
		...
		function triggerFields(kind) {
			if (kind === "approval") return ["approvalSound", "approvalVolume", "approvalRepeat", "approvalInterval"];
			if (kind === "completion") return ["completionSound", "completionVolume", "completionRepeat", "completionInterval"];
			return ["questionSound", "questionVolume", "questionRepeat", "questionInterval"];
		}
		...
		function apply(ctx) {
			...
			*  - bgColor -> theme token override on --dsw-alias-bg-base (the app
			*    frame's background), 单一背景色 both palettes.
			*  - bgImage  -> the app frame turns transparent and a fixed
			*    body::before layer carries the image at bgOpacity, with the base
			*    color behind it. Panels keep their own opaque surface tokens, so
			*    content stays readable.
			*/
			function applyBackground(bg) {
				var color = typeof bg.bgColor === "string" ? bg.bgColor.trim() : "";
				var image = typeof bg.bgImage === "string" ? bg.bgImage.trim() : "";
				var opacity = typeof bg.bgOpacity === "number" ? Math.min(Math.max(bg.bgOpacity, 0), 1) : 1;
				// 1) frame background via theme token override (stacked over the
				// active light/dark theme; follows theme switches).
				if (bgOverrideDispose !== null) {
					bgOverrideDispose();
					bgOverrideDispose = null;
				}
				var frameBg = image !== "" ? "transparent" : color !== "" ? color : null;
				if (frameBg !== null && ctx.theme !== void 0) {
					bgOverrideDispose = ctx.theme.overrideTokens("dsh-sound-notify", {
						"--dsw-alias-bg-base": { light: frameBg, dark: frameBg }
					});
				}
				// 2) body layer: base color + image with opacity.
				...
				if (image !== "") {
					...
					var base = color !== "" ? color : "#000000";
					rules += "body{background-color:" + base + ";background-image:" + url + ";background-size:cover;background-position:center;background-attachment:fixed}";
					rules += "body::before{content:\"\";position:fixed;inset:0;z-index:0;pointer-events:none;opacity:" + opacity + ";background-image:" + url + ";background-size:cover;background-position:center;background-attachment:fixed}";
				} else if (color !== "") {
		...
		var GROUPS = [
			{ key: "question", label: "提问", soundField: "questionSound", volumeField: "questionVolume", repeatField: "questionRepeat", intervalField: "questionInterval" },
			{ key: "approval", label: "审批", soundField: "approvalSound", volumeField: "approvalVolume", repeatField: "approvalRepeat", intervalField: "approvalInterval" },
			{ key: "completion", label: "任务完成", soundField: "completionSound", volumeField: "completionVolume", repeatField: "completionRepeat", intervalField: "completionInterval" }
		];
		var SOUND_OPTIONS = [
			["rising", "上扬"],
			["falling", "下行"],
		...
		/** Background personalization group: color, image URL, image opacity. */
		function BackgroundGroup({ value, setField, unsetField }) {
			...
			var opacity = typeof value.bgOpacity === "number" ? value.bgOpacity : 1;
			return el("div", null, [
				...
				el("div", { className: "dshsn_field" }, [
					el("span", { className: "dshsn_fieldLabel" }, "图片不透明度"),
					el("div", { className: "dshsn_row" }, [
						el("input", {
							type: "range",
							className: "dshsn_slider",
		...
		function SoundSection({ scope }) {
			...
			return el("div", { className: "dshsn_section" }, [
				...
				el("p", { className: "dshsn_note" },
					"背景色与主题（设置 → 外观）互补；设置图片后主背景变为透明以显示图片，各面板保持不透明。提示音每个触发条件可独立设置音效、音量、重复次数与间隔；试听按当前配置播放，改动自动保存。"),
