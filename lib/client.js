...
// or completes a turn. Per-trigger sound preset and volume are configurable in
// real time through the DSH settings system (namespace "sound-notify"):
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*  repeats of one sequence are never blocked by it. */
		...
		var inject = ["sessions", "settingsScope", "connection", "remote", "slots"];
		...
		function apply(ctx) {
			...
			var applySettings = function () {
				...
				if (snap && snap.value && typeof snap.value === "object") cfg = snap.value;
			};
		...
		/** The settings.section page for this plugin. */
		function SoundSection({ scope }) {
			...
			return el("div", { className: "dshsn_section" }, [
				...
				el("p", { className: "dshsn_note" },
					"每个触发条件可独立设置音效、音量、重复次数与间隔；试听按当前配置播放，改动自动保存。"),
