...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** Reused <audio> element for custom (uploaded) sounds. */
		...
		*  first interaction so later chimes are never blocked. */
		...
		/** Play one full repeat-sequence on a RUNNING context (first ring + timers).
		*  Only called with a running context so `currentTime` is live — scheduling
		*  against a suspended (frozen-clock) context clips or drops the tones. */
			...
			for (var i = 1; i < count; i++) {
				pendingTimers.push(setTimeout(function () {
					...
					if (c.state === "suspended") {
						// Mid-sequence suspend (long gaps): resume first, then ring.
						c.resume().then(function () {
							if (c.state === "running") ringOnce(c, preset, vol);
						}).catch(function () {});
						return;
					}
					ringOnce(c, preset, vol);
				}, i * gap));
			}
		}
		...
		*  repeats of one sequence are never blocked by it. */
		...
		/** Play a custom audio file (uploaded URL) with repeat/interval. */
		function playCustomAudio(url, volume, repeat, interval) {
			...
			var playOnce = function () {
				if (customAudioEl === null) customAudioEl = new Audio();
				var el = customAudioEl;
				try {
					el.pause();
					el.currentTime = 0;
					el.src = url;
		...
		function apply(ctx) {
			...
			*  shows the theme background instead of pure black. */
			var defaultBg = null;
			function captureDefaultBg() {
				if (defaultBg !== null || typeof document === "undefined") return;
				var value = getComputedStyle(document.body).getPropertyValue("--dsw-alias-bg-base").trim();
				defaultBg = value !== "" ? value : null;
			}
			/**
		...
		/** Background personalization group: color, image (URL or local file), image opacity. */
